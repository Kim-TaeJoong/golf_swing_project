...
def _process_landmarks(landmarks_path: str) -> pd.DataFrame:
    ...
    '''
    feat_df.loc[feat_df['r_elbow'] < 30, 'r_elbow'] = np.nan
    feat_df.loc[feat_df['l_elbow'] < 30, 'l_elbow'] = np.nan
    
    # 가짜 데이터(NaN)로 뚫린 구멍을 앞뒤 프레임의 정상적인 흐름으로 자연스럽게 메꾸기(보간)
    feat_df[ANGLE_COLS] = feat_df[ANGLE_COLS].interpolate(method='linear', limit_direction='both')
    '''
    # 각도 스무딩
...
def print_max_difference(df: pd.DataFrame, feature_name: str = 'r_elbow'):
    """
    정렬된 DataFrame에서 특정 각도의 오차(절댓값)가 가장 큰 지점을 찾아 출력합니다.
    """
    diff_col = f'diff_{feature_name}'
    
    if diff_col not in df.columns:
        print(f"경고: {diff_col} 컬럼이 존재하지 않습니다.")
        return

    # NaN 데이터 제외하고 절댓값 오차가 가장 큰 인덱스 찾기
    clean_df = df.dropna(subset=[diff_col]).copy()
    clean_df['abs_diff'] = clean_df[diff_col].abs()
    
    if clean_df.empty:
        print(f"경고: {diff_col} 비교할 수 있는 유효한 데이터가 없습니다.")
        return

    max_idx = clean_df['abs_diff'].idxmax()
    max_row = clean_df.loc[max_idx]

    print("\n" + "="*50)
    print(f"🚨 최대 오차 발생 지점 분석: {feature_name} 🚨")
    print("="*50)
    print(f"▶ 데이터 인덱스 : {max_idx}")
    print(f"▶ 프로 프레임   : {int(max_row['pro_frame'])} 프레임")
    print(f"▶ 유저 프레임   : {int(max_row['user_frame'])} 프레임")
    print("-" * 50)
    print(f"프로 각도       : {max_row[f'pro_{feature_name}']:.2f} 도")
    print(f"유저 각도       : {max_row[f'user_{feature_name}']:.2f} 도")
    print(f"실제 각도 차이  : {max_row[diff_col]:.2f} 도")
    print("="*50 + "\n")

def get_user_events_from_dtw(
    pro_path,
    user_path,
    pro_events,
    feature='r_wrist_y',
    ...
    """
    프로 이벤트를 DTW를 통해 사용자 이벤트로 투영
    """

    # 기존 코드 재사용
    if use_landmarks:
        pro_df, _ = _process_landmarks(pro_path)
        user_df, _ = _process_landmarks(user_path)
    else:
    ...
    for event_name, pro_frame in pro_events.items():

        matches = pro_df.index[
            pro_df['frame'] == pro_frame
        ]

        if len(matches) == 0:
            continue

        pro_idx = int(matches[0])

        user_matches = [
            user_idx
            for p_idx, user_idx in path
            if p_idx == pro_idx
        ]
        ...
        user_idx = int(round(np.median(user_matches)))

        user_events[event_name] = int(
            user_df.loc[user_idx, 'frame']
        )
...
if __name__ == "__main__":
    ...
    print(aligned_df[['pro_frame', 'user_frame', 'pro_l_elbow', 'user_l_elbow', 'diff_l_elbow']].head(10).to_string(index=False))
    ...
    plot_alignment_v2(PRO_LM, USER_LM, feature='spine_angle', events=events, use_landmarks=True, save_path=PLOT_OUT)

    user_events = get_user_events_from_dtw(
    PRO_LM,
    USER_LM,
    events,
    feature='r_wrist_y',
